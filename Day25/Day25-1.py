...
def get_all_output(computer):
    s = ""
    while computer.has_output():
        s += chr(computer.get_output())
    return s


def print_output(computer):
    print(get_all_output(computer))
...
def main():
    program = list(map(int, [line.rstrip().split(',') for line in open('Day25.txt')][0]))
    ...
    while True:
        ...
        if command == "solve":
            break
        enter_command(computer, command)

    inventory = ["mouse", "spool of cat6", "hypercube", "sand", "antenna", "mutex", "boulder", "astronaut ice cream"]

    for item in inventory:
        enter_command(computer, "drop " + item)
        computer.run()

    enter_command(computer, "inv")
    computer.run()
    get_all_output(computer)

    savestate = deepcopy(computer)

    for length in range(len(inventory)):
        permutations = list(itertools.combinations(inventory, length+1))

        for permutation in permutations:
            computer = deepcopy(savestate)

            for item in permutation:
                enter_command(computer, "take " + item)
                computer.run()

            get_all_output(computer)
            enter_command(computer, "south")
            computer.run()

            output = get_all_output(computer)

            if output.find("ejected") == -1:
                print(output)
                break
