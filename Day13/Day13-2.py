...
computer = Computer(list(map(int, [line.rstrip().split(',') for line in open('Day13.txt')][0])))
computer.memory[0] = 2
...
while True:
    ...
    if computer.state == ProcessorState.IDLE:
        print(score)
        break

    joystick = max(min(ball_x-paddle_x, 1), -1)
