...
def square_fits_in_beam(xx, yy):
    c = Computer(list(map(int, [line.rstrip().split(',') for line in open('Day19.txt')][0])))
    c.add_input(xx)
    c.add_input(yy)
    c.run()
    return c.get_output()
...
def main():
    ...
    program = list(map(int, [line.rstrip().split(',') for line in open('Day19.txt')][0]))

    while True:
        computer = Computer(program.copy())
        if x > y:
            computer.add_input(y_x)
            computer.add_input(y)
            computer.run()
            if computer.get_output():
                if square_fits_in_beam(y_x + (width-1), y - (width-1)):
                    print(y_x*10000 + y-(width-1))
                    break
                y += 1
            else:
                y_x += 1
            ...
            computer.add_input(x_y)
            computer.run()
            if computer.get_output():
                if square_fits_in_beam(x - (width-1), x_y + (width-1)):
                    print((x-(width-1)) * 10000 + x_y)
                    break
                x += 1
            else:
                x_y += 1
