...
def find_routes_dfs(state, keys):
    ...
    if best is not None:
        first = keys[state["key"]][1][0]

        if len(collected_keys) > 2:
            if state["steps"] + len(first[1]) // 2 > best_steps[len(collected_keys)]:
                return

    for next_key in keys[state["key"]][1]:
...
def main():
    ...
    find_routes_dfs({"key": '@', "steps": 0, "best_steps": [0], "collected_keys": '@'}, keys)
